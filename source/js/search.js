(function ($, window, instantsearch) {
    var CONFIG = {
        root: '/',
        algolia: {
            applicationID: 'L5Y6TGO99A',
            apiKey: '5b03f938f8555900ee9e0a33cfd27369',
            indexName: 'matosiki.site',
            hits: { "per_page": 10 },
    ...
    [
        ...
        instantsearch.widgets.hits({
            ...
            templates: {
                item: function (data) {
                    var keys = (data.permalink + '').split("/")
                    var title = keys[0]
                    if (keys.length >= 2) {
                        title = keys[keys.length - 2]
                    }
                    return (
